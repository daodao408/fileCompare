            ...
            <uploader :options="uploaderFolderOptions"
              ...
              <uploader-drop>
                ...
                <div class="upload-info"
                     ...
                     v-show="uploadFoldFlag">
                  <i></i>
                  <span :title="`已选择 ${folderFileName} 文件夹`">已选择 {{folderFileName}} 文件夹</span>
                </div>
            ...
            <uploader :options="uploaderFileOptions"
              ...
              <uploader-drop>
                ...
                <div class="upload-info"
                     ...
                     v-show="uploadFileFlag">
                  <i></i>
                  <span :title="`已选择 ${compareFileName} 文件`">已选择 {{compareFileName}} 文件</span>
                </div>
                <img class="circle-icon"
...
export default class Main extends Vue {
  ...
  compareFileList: any = [] // 对比文件数组
  folderFileName: String = '' // 上传文件夹文件名称
  compareFileName: String = '' // 对比文件数组
  uploadFoldFlag: Boolean = false // 上传文件夹标志量  已选择   未选择
  uploadFileFlag: Boolean = false // 对比文件标志量  已选择   未选择
  ...
  onFolderAdded(files, fileList) {
    ...
    for (let item of files) {
      ...
      excelSizeTotal += item.size
    }
    if (excelSizeTotal > 1024 * 1024 * 20) {
      this.$message({
        type: 'warning',
        message: '上传文件夹大小超过20MB'
      })
    ...
    if (this.folderFileList.length === 0) {
      ...
      this.folderFileName = files[0].relativePath.split('/')[0]
  ...
  onFileAdded(files, fileList) {
    ...
    if (fileSuffix !== 'doc' && fileSuffix !== 'docx') {
      ...
      this.compareFileName = fileList[0].name
      let params = new FormData()
